...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    //1.先查询所有用户
        ...
        //2.在内存中判断是否包含要求的标签
        Gson gson = new Gson();
        //代替了一次for循环，每一次的过滤如果返回false当前的user对象就会被滤掉
        return userList.stream().filter(user -> {
            ...
            tempTagNameSet= Optional.ofNullable(tempTagNameSet).orElse(new HashSet<>());//java8特性，相当于判空，是空的就用后边的替换
    ...
    private List<User> searchUsersByTagsBYSQL(List<String> tagNameList){
        ...
        QueryWrapper<User> queryWrapper =new QueryWrapper<>();
        //拼接and查询,用循环的方式，每一次都会返回一个queryWrapper然后再做like查询，每一次循环相当于一次and
